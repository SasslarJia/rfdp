...
import torch
#
from Itrt_Extn.ItrtQryExtn import ItrtQryExtn
...
def test_rfdp():
    ...
    # oput_rslts = RfdpCtofOtcm(mdl_iput=mdl_iput, fit_cstr=HyDiffFC(0.1), sol_proc=ItrtExpGrw(),
    #                           incl_qry=False)(iput_qries=list([[1], [3], [5], [6], [7], [8]]), iput_num=ele_num)
    #
    ...
    #
    oput_rslts = ItrtQryExtn(
        RfdpSuplExtn(mdl_iput=mdl_iput, fit_cstr=HyDiffFC(0.05), sol_proc=ItrtExpGrw(), incl_qry=False),
        20)([[1], [3], [5], [6], [7], [8]], 3)
    #
